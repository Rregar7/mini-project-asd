...
# Searching Space =============================================
    ...
    def fibonacci_search_by_id(self, id_laporan):
        ...
        while fib_m > 1:
            ...
            if current_node.id_laporan < id_laporan:
                ...
                offset = i
            elif current_node.id_laporan > id_laporan:
                fib_m = fib_m_minus_1
        ...
        if fib_m_minus_1 and self.head and self.head.id_laporan == id_laporan:
            return self.head

        return None

    def jump_search_by_nama(self, nama_pelapor):
        ...
        while current_node and current_node.pelapor < nama_pelapor:
        ...
        while prev_node and prev_node.pelapor < nama_pelapor:
            prev_node = prev_node.next

        if prev_node and prev_node.pelapor == nama_pelapor:
            return prev_node
        else:
            return None

    def search_by_id(self, id_laporan):
        result = self.fibonacci_search_by_id(id_laporan)
        if result:
            print("+==========================================+")
            print("Data ditemukan :")
            print("ID :", result.id_laporan)
            print("Pelapor :", result.pelapor)
            print("Judul Laporan :", result.judul_laporan)
            print("Isi Laporan :", result.isi_laporan)
            print("Tanggal Laporan :", result.tanggal_laporan)
            print("+==========================================+")
        else:
            print("+==========================================+")
    ...
    def search_by_nama(self, nama_pelapor):
        result = self.jump_search_by_nama(nama_pelapor)
        if result:
            print("+==========================================+")
            print("Data ditemukan :")
            print("ID :", result.id_laporan)
...
# Sorting Space =============================================
    ...
    def merge_sort_desc_pelapor(self, data):
        if len(data) > 1:
            ...
            self.merge_sort_desc_pelapor(left)
            self.merge_sort_desc_pelapor(right)
    ...
    def update_sorted_data(self, sorted_data):
        ...
        for i in range(len(sorted_data)):
            current_node.id_laporan = sorted_data[i][0]
        ...
        def combined_sorted_id(self, data):
            if len(data) > 1:
                mid = len(data) // 2
                left = data[:mid]
                right = data[mid:]

                self.combined_sorted_id(left)
                self.combined_sorted_id(right)

                i = j = k = 0

                def value_id_laporan(laporan):
                    return laporan[0]

                while i < len(left) and j < len(right):
                    if value_id_laporan(left[i]) < value_id_laporan(right[j]):
                        data[k] = left[i]
                        i += 1
                    else:
                        data[k] = right[j]
...
laporan.create_laporan_head(id_laporan=None, pelapor="Regar Siregar", judul_laporan="Pemalsuan Data", isi_laporan="Ada data yang dipalsukan", tanggal_laporan=tgl_lapor)
laporan.create_laporan_head(id_laporan=None, pelapor="Mirza Situmorang", judul_laporan="Pelucutan Fasilitas", isi_laporan="Fasilitas yang dilucuti", tanggal_laporan=tgl_lapor)
laporan.create_laporan_head(id_laporan=None, pelapor="Arey", judul_laporan="Pinjol Lari", isi_laporan="Nasabah tidak ingin membayar", tanggal_laporan=tgl_lapor)
laporan.create_laporan_head(id_laporan=None, pelapor="Icam", judul_laporan="Penyalahgunaan Tempat", isi_laporan="Orang Tidur Sembarangan", tanggal_laporan=tgl_lapor)
laporan.create_laporan_head(id_laporan=None, pelapor="Rizky", judul_laporan="Penipuan Transaksi", isi_laporan="Penjual membawa lari uang", tanggal_laporan=tgl_lapor)
...
while True:
    ...
    if pilihan == '1':
        ...
        print('5. Sort by ID (Ascending)')
        pilihan_read = input('Pilih Jenis Sort (1-5) (Enter Untuk Keluar) : ')
        cls()
        ...
        if pilihan_read == '3':
            ...
            laporan.combined_sorted_id(laporan_data)
            laporan.update_sorted_data(laporan_data)
            cls()
            laporan.read_laporan()
        ...
        print("2. Cari berdasarkan Nama Pelapor")
        print("+==========================+")

        pilihan_search = input("Pilih jenis pencarian (1-2): ")

        if pilihan_search == '1':
            id_laporan = int(input("Masukkan ID Laporan yang dicari: "))
            cls()
            laporan.search_by_id(id_laporan)
        elif pilihan_search == '2':
            nama_pelapor = input("Masukkan Nama Pelapor yang dicari: ")
            cls()
            laporan.search_by_nama(nama_pelapor)
        else:
            print("Pilihan tidak valid.")
    else:
        print("+==============================================+")
